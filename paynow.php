...
if(isset($_SESSION['usr'])){
    ...
    $t_id=$r_usr[3];
    $d_id=$r_usr[2];
    $stn=mysqli_query($con,("select * from tal_info where id='$t_id'");
    $tn=mysqli_fetch_row($stn);
    $svn=mysqli_query($con,("select * from vil_info where id='$v_id'");
    $vn=mysqli_fetch_row($svn);
}
else{
...
if(isset($_POST['sub']))
{
   ...
   $amt=$_POST['amt'];
   $dt=date('Y-m-d');
   $rsn="Payment of Village TAX";
   $chkacno=mysqli_query($con,("select * from vb_usr where acc_num='$ac' and em='$em'");
    if(mysqli_num_rows($chkacno)>0){
        $r_amt=mysqli_fetch_row($chkacno);
        if($r_amt[6]<$amt){
            header("location:paynow.php?pid=$pid&fail=2");
        }
        else{
            $ins_tran=mysqli_query($con,("insert into cash_tranreq values('','$r_usr[5]','$ac','Village','111-111-001','$amt','$dt','$pid','$rsn',0)");
            if($ins_tran>0){
                $up=mysqli_query($con,("update village_tax set st=1 where id='$pid'");
                if($up>0){
    ...
    {
        header("location:paynow.php?pid=$pid&fail=1");
    }
}
?>
<!DOCTYPE html>
    ...
    <div class="box col-md-12">
        <div class="box-inner">
            <div class="box-header well">
                <h2><i class="glyphicon glyphicon-info-sign"></i> PAY YOUR VILLAGE TAX HERE</h2>
            ...
            <div class="box-content row">
                <div class="col-lg-12 col-md-12">                      
                        <div class="col-lg-6">
                            <?php
                            $sel_tax=mysqli_query($con,("select * from village_tax where usr_id='$usr' and id='".$_GET['pid']."'");
                            $r_tax=mysqli_fetch_row($sel_tax);
                                ?>
                            <table class="table table-bordered table-condensed table-hover table-responsive">
                                <tr>
                                    <td>Tax Book Number</td>
                                    <td><input type="text" name="bn" value="<?php echo $r_tax[5] ?>" class="form-control" /></td>
                                </tr>                                
                                <tr>
                                    <td>Amount</td>
                                    <td><input type="text" name="amt" value="<?php echo $r_tax[15] ?>"  class="form-control" /></td>
                                </tr>
                                <tr>
                                    <td colspan="2"><center><b>ENTER YOUR BANK DETAILS</b></center></td>
                                </tr>
                                <tr>
                                    <td>EMAIL</td>
                                ...
                                <tr>
                                    <td colspan="2"><center><input type="submit" name="sub" value="Pay Amount" class="btn btn-success" /></center></td>
                                </tr>
                            </table>
                            <?php
                                            if(isset($_GET['fail'])){
                                                if($_GET['fail']==1)
                                                {
                                                echo"<font color='red'><b>Invalid Details...</b></font>";
                                                }
                                                if($_GET['fail']==2)
                                                {
                                                echo"<font color='red'><b>Insufficient Fund</b></font>";
                                                }
                                            }
                                            ?>
                        </div>
                    <div class="col-lg-6">
                        <table class="table table-bordered table-condensed table-hover table-responsive">
                            <tr>
                                <td colspan="5">VILLAGE TAX PAID INFO</td>
                            </tr>
                            <tr>
                                <td>#</td>
                                <td>BOOK NUMBER</td>
                        ...
                        <?php
                        $i=1;
                        $sel_tax=mysqli_query($con,("select * from village_tax where usr_id='$usr' order by id desc");
                        while($r_tax=mysqli_fetch_row($sel_tax))
                        {
                            ?>
                            <tr>
                                <td><?php echo $i ?></td>
                                <td><?php echo $r_tax[5] ?></td>
                                ...
                                <td><center>
                                    ...
                                    {
                                        ?>
                                    <a href="paynow.php?pid=<?php echo $r_tax[0] ?>"><span class="btn btn-sm btn-danger">PAY</span></a>
                                    <?php
                                    }
                                    if($r_tax[16]==1)
                                    {
                                    ...
                                    {
                                        ?>
                                    <span class="btn btn-sm btn-success">Print</span>
                                    <?php
                                    }
                                    ?>
