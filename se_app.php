...
if(isset($_POST['sub']))
{
    ...
    $up=$_FILES['up']['name'];
    $dt=date('Y-m-d');
    if(move_uploaded_file($_FILES['up']['tmp_name'], getcwd()."\\usr_app\\".$_FILES['up']['name'])){
        $ins_app=mysqli_query($con,"insert into sec_app values('','$at','$ad','$up','$usr','$r_usr[5]','$dt','0')");
        if($ins_app>0){
            header("location:se_app.php");
        }
    }
}
?>
        ...
        <div class="box-inner">
            ...
            <div class="box-content row">
                <div class="col-lg-12 col-md-12">
                        <div class="col-lg-6">
                            <h4>POST APPLICATION HERE</h4>
                            <table class="table table-bordered table-condensed table-hover">
                                <tr>
                                    <td>Application Title</td>
                                    <td><input type="text" name="at" class="form-control" /></td>
                                </tr>
                                <tr>
                                ...
                                <tr>
                                    <td>Attach Application</td>
                                    <td><input type="file" name="up" class="form-control" /></td>
                                </tr>
                                <tr>
                                    <td colspan="2"><center><input type="submit" name="sub" value="POST APPLICATION" class="btn btn-success" /></center></td>
                                </tr>
                            </table>
                        </div>
                        <div class="col-lg-6">
                            ...
                            <?php
                            $sel_app=mysqli_query($con,"select * from sec_app where app_by='$usr'");
                            if(mysqli_num_rows($sel_app)>0){
                                ?>
                            ...
                            <?php
                            $i=1;
                                while($r_app=mysqli_fetch_row($sel_app))
                                {
                                    ?>
                                <tr>
                                    <td><?php echo $i ?></td>
                                    <td><?php echo $r_app[1] ?></td>
                                    <td><?php echo date("d M Y (D)", strtotime($r_app[5])); ?></td>
                                    <td>
                                        <?php
                                        if($r_app[7]==0){
                                            echo"Waiting...";                                            
                                            ...
                                            echo"<span onclick='sho_rp($i)'>Readed</spn>";
                                        }
                                        ?>
                                    </td>
                                </tr>
                                <tr>
                                    <td></td>
                                    <td colspan="3">
                                        <span id="rp<?php echo $i ?>" style="display: none">
                                     <?php
                                    $sel_rp=mysqli_query($con,"select * from app_rply where appid='$r_app[0]'");
